...
const NavItem = ({ label, href = "#", children }: NavItemProps) => {
  ...
  return (
    ...
    >
      {children ? (
        <button className="flex items-center gap-1 text-white hover:text-yellow-400 font-semibold text-[13px] uppercase tracking-wider transition-colors py-6">
          {label}
          <ChevronDown size={14} className={`transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
        </button>
      ) : (
        <Link href={href} className="flex items-center gap-1 text-white hover:text-yellow-400 font-semibold text-[13px] uppercase tracking-wider transition-colors py-6">
          {label}
        </Link>
      )}
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-10">
                ...
                <button onClick={() => setMobileMenuOpen(false)} className="text-slate-400 hover:text-white transition-colors" aria-label="Close menu">
                  <X size={24} />
                </button>
              </div>

              <div className="flex-1 space-y-6">
                <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block text-base font-black text-white hover:text-yellow-400 transition-colors">Home</Link>

                <div className="space-y-4">
                  <p className="text-[13px] uppercase tracking-[0.3em] text-slate-600 font-bold">About</p>
                  <Link href="/about" onClick={() => setMobileMenuOpen(false)} className="block text-base font-bold text-slate-400 hover:text-white">Who We Are</Link>
                  <Link href="/team" onClick={() => setMobileMenuOpen(false)} className="block text-base font-bold text-slate-400 hover:text-white">Our Doctors</Link>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {BRANCHES.map(b => (
                    <Link key={b.name} href={b.href} onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 text-base font-bold text-slate-400 hover:text-white">
                      <MapPin size={16} className="text-yellow-500" />{b.name}
                    </Link>
                  ))}
                </div>
              ...
              <div className="pt-8 border-t border-slate-900 mt-auto">
                ...
                <div className="flex justify-center gap-6 text-slate-500">
                  <a href="tel:+919236952369" className="hover:text-yellow-400 transition-colors"><Phone size={20} /></a>
                  <Link href="/contact" className="hover:text-yellow-400 transition-colors"><MapPin size={20} /></Link>
                </div>
              </div>
            </motion.div>
          </>
