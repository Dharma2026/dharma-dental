...
const VIDEO_REVIEWS: VideoReview[] = [
  {
    ...
    duration: "2:15",
    videoUrl: "https://youtu.be/1MTyCvS05V4?si=S4gRnG3IB8XG5s_w" // 👈 Replace
  },
  {
    id: 2,
...
].map(v => ({ ...v, youtubeId: v.videoUrl ? getYouTubeId(v.videoUrl) : '' }));
...
const VideoModal: React.FC<VideoModalProps> = ({ youtubeId, title, onClose }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
  ...
  >
    ...
    >
      ...
      <button
        onClick={onClose}
        className="absolute top-3 right-3 w-9 h-9 bg-black/70 hover:bg-red-600 rounded-full flex items-center justify-center text-white transition-colors"
...
export default function Testimonials() {
  ...
  const [activeVideo, setActiveVideo] = useState<VideoReview | null>(null); // 👈 modal state
                          ...
                          <div className="relative aspect-video rounded-3xl overflow-hidden border border-slate-800 mb-6 shadow-2xl bg-slate-900">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                              <motion.button
                                ...
                                onClick={() => setActiveVideo(video)} // 👈 opens modal
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between mt-12 gap-8 border-t border-slate-900 pt-8">
                    <div className="flex-1 w-full max-w-xs bg-slate-900 h-1 rounded-full overflow-hidden">
                      <motion.div 
                        ...
                        initial={{ width: "0%" }}
                        animate={{ width: `${((videoIndex + 1) / VIDEO_REVIEWS.length) * 100}%` }}
                      />
                    </div>
